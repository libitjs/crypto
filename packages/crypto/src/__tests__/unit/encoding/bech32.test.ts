...
function decode(expect: string, addr: string): [string, number, Buffer] {
  ...
  if (version === 0 && hash.length !== 20 && hash.length !== 32) throw new Error('Malformed witness program.');
...
function encodeManual(hrp: string, version: number, hash: Buffer, lax?: boolean) {
...
function decodeManual(expect: string, addr: string, lax = false): [string, number, Buffer] {
  const [hrp, data] = bech32.deserialize(addr);

  if (!lax) {
    if (hrp !== expect || data.length < 1 || data[0] > 16) throw new Error('Invalid bech32 prefix or data length.');
  }

  const hash = bech32.convertBits(data.slice(1), 5, 8, false);

  if (!lax) {
    if (hash.length < 2 || hash.length > 40) throw new Error('Invalid witness program size.');
  }

  if (!lax) {
    if (data[0] === 0 && hash.length !== 20 && hash.length !== 32) throw new Error('Malformed witness program.');
  }
