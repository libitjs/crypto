...
const algs = [
  {
    ...
    keyLen: 16,
    ivLen: 16,
    ids: ['AES-128-ECB', 'AES-128-CBC', 'AES-128-CTR', 'AES-128-CFB', 'AES-128-OFB', 'AES-128-GCM'],
  },
  {
    name: 'AES-192',
    keyLen: 24,
    ivLen: 16,
    ids: ['AES-192-ECB', 'AES-192-CBC', 'AES-192-CTR', 'AES-192-CFB', 'AES-192-OFB', 'AES-192-GCM'],
  },
  {
    name: 'AES-256',
    keyLen: 32,
    ivLen: 16,
    ids: ['AES-256-ECB', 'AES-256-CBC', 'AES-256-CTR', 'AES-256-CFB', 'AES-256-OFB', 'AES-256-GCM'],
  },
  {
    name: 'ARC2',
    keyLen: 8,
...
function _encipher(ctx: Cipher, gcm: boolean, key: Buffer, iv: Buffer, data: Buffer) {
  ctx.init(key, iv);
  ...
  return Buffer.concat([ctx.update(data), ctx.final(), gcm ? ctx.getAuthTag() : Buffer.alloc(0)]);
}

function _decipher(ctx: Cipher, gcm: boolean, key: Buffer, iv: Buffer, data: Buffer) {
  ctx.init(key, iv);
  ...
  for (const alg of algs) {
    describe(alg.name, function () {
      for (const id of alg.ids) {
        ...
        const file = path.resolve(__dirname, '..', 'data', 'ciphers', `${id.toLowerCase()}.json`);
